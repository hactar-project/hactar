...
(test tool-call-command-tests
  "Test that execute-command handles /tool-call with positional, named, normalized, and coerced args."
  (clrhash hactar::*defined-tools*)
  (let ((orig-perm-fn #'hactar::resolve-permission))
    (setf (fdefinition 'hactar::resolve-permission)
          (lambda (tool-name args) (declare (ignore tool-name args)) :allow))
    (unwind-protect
         (progn
           ;; Define a test tool with various types
           (eval '(hactar::deftool cmd-test-tool ((name :string :required t)
                                                 (count :number :required t)
                                                 (flag :boolean :required t)
                                                 (optional-param :string :required nil))
                    :description "Command test tool"
                    :permissions :auto
                    (format nil "name:~A count:~A flag:~A opt:~A"
                            (getf hactar::args :name)
                            (getf hactar::args :count)
                            (getf hactar::args :flag)
                            (getf hactar::args :optional-param))))

           ;; 1. Test positional args only
           (let ((output1 (with-output-to-string (*standard-output*)
                            (hactar::execute-command "/tool-call" '("cmd_test_tool" "alice" "42" "true")))))
             (is (search "Result: name:alice count:42 flag:T opt:NIL" output1)))

           ;; 2. Test named args only
           (let ((output2 (with-output-to-string (*standard-output*)
                            (hactar::execute-command "/tool-call" '("cmd_test_tool" "name=bob" "count=10" "flag=false")))))
             (is (search "Result: name:bob count:10 flag:NIL opt:NIL" output2)))

           ;; 3. Test mixed named and positional args
           (let ((output3 (with-output-to-string (*standard-output*)
                            (hactar::execute-command "/tool-call" '("cmd_test_tool" "flag=true" "charlie" "5")))))
             (is (search "Result: name:charlie count:5 flag:T opt:NIL" output3)))

           ;; 4. Test underscore to hyphen normalization in named keys
           (let ((output4 (with-output-to-string (*standard-output*)
                            (hactar::execute-command "/tool-call" '("cmd_test_tool" "optional_param=yay" "alice" "100" "false")))))
             (is (search "Result: name:alice count:100 flag:NIL opt:yay" output4))))
      (setf (fdefinition 'hactar::resolve-permission) orig-perm-fn))))
