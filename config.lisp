    ...
    (when (probe-file config-file)
      ...
      (handler-case
          (let* ((config-content (uiop:read-file-string config-file))
            ...
            (let ((paths-table (gethash "paths" config-data)))
		      ...
		      (hactar-repo-dir (gethash "hactar_repo_dir" paths-table))
                      ...
                      (docs-import (or (gethash "docs_import" paths-table)
                                       (gethash "docs_import_path" paths-table)))
                      (starters-import (or (gethash "starters_import" paths-table)
                                           (gethash "starters_import_path" paths-table)))
		  ...
		  (when hactar-repo-dir (setf *hactar-repo-dir* hactar-repo-dir))
                  ...
                  (when docs-import (setf *docs-import-path* docs-import))
                  (when starters-import (setf *starters-import-path* starters-import))
            ...
            (let ((api-keys-table (gethash "api_keys" config-data)))
              (when api-keys-table
                (let ((openai (gethash "openai" api-keys-table))
                      ...
                      (nvidia-nim (or (gethash "nvidia_nim" api-keys-table)
                                      (gethash "nvidia-nim" api-keys-table))))
                  ...
                  (when nvidia-nim (setf llm:*nvidia-nim-api-key* nvidia-nim)))))
            ;; Analyzer settings
