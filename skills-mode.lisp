...
(define-command skills.list (args)
  ...
  :xml (lambda (args)
         ...
         (with-output-to-string (s)
           (format s "<skills>~%")
           (dolist (skill (list-skills))
             (format s "  <skill>~%    <name>~A</name>~%    <description>~A</description>~%  </skill>~%"
                     (gethash "name" skill) (gethash "description" skill)))
           (format s "</skills>~%")))
  :org-mode (lambda (args)
              ...
              (with-output-to-string (s)
                ...
                (dolist (skill (list-skills))
                  (format s "| ~A | ~A |~%" (gethash "name" skill) (gethash "description" skill)))))
  ...
  :acp (lambda (cmd-args)
         ...
         (let ((skills (list-skills)))
           `(("text" . ,(format nil "~A skills available." (length skills)))
             ("data" . ,(coerce (loop for s in skills
                                      collect `(("name" . ,(gethash "name" s))
                                                ("description" . ,(gethash "description" s))))
                                'vector))))))
